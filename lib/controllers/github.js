...
module.exports = Router()
  .get('/login', async (req, res) => {
    ...
    res.redirect(
      `https://github.com/login/oauth/authorize?client_id=${process.env.CLIENT_ID}&scope=user&redirect_uri=http://localhost:7890/api/v1/auth/login/callback/i`
    );
  })
  .get('/login/callback', async (req, res) => {
    ...
    res.cookie(process.env.COOKIE_NAME, process.env.JWT_SECRET, {
      httpOnly: true,
